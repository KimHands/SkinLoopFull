...
export default function HomePage(){
 ...
 return <div className="page stack">
  ...
  <section className="grid-2">
   <div className="card"><p className="caption">오늘의 체크인</p><h1 className="page-title" style={{marginBottom:10}}>{total?`${total}일째 기록 중`:"첫 기록을 시작해 볼까요?"}</h1><p className="subtle">작은 기록이 쌓이면 반복적으로 함께 나타나는 패턴을 살펴볼 수 있어요.</p><Link className="btn btn-primary btn-block" href="/record">오늘 기록하기</Link></div>
   <div className="card"><p className="caption">분석 진행도</p>{total<7?<><h2 style={{fontSize:18}}>앞으로 {need}일 더 기록하면 분석이 시작돼요</h2><div style={{height:10,borderRadius:9,background:"var(--border)",overflow:"hidden",margin:"20px 0 8px"}}><div style={{width:`${Math.min(total/7*100,100)}%`,height:"100%",background:"var(--primary)"}}/></div><p className="caption">{total}일 기록 / 7일</p></>:<><h2 style={{fontSize:18}}>새 인사이트를 확인할 수 있어요</h2><p className="subtle">최근 기록을 기준으로 연관 가능성이 높은 생활 습관을 살펴보세요.</p><Link className="btn btn-secondary btn-block" href="/analyzing">인사이트 보기</Link></>}</div>
  </section>
